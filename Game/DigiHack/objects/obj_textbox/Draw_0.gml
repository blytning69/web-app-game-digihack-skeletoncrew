...
textbox_y = camera_get_view_y(view_camera[0]) + camera_get_view_height(view_camera[0]) / 2;
ai_input_key = keyboard_check_pressed(ord("E"));
draw_set_font(font_menu);

// Update ukuran berdasarkan camera
...
// Handle AI chat input (SEPARATE from accept_key)
...
if accept_key && !input_active {
    if draw_char == text_length[page] {
        ...
        if page < page_number - 1 {
            ...
            // Check if we're showing options
            if (option_number > 0 && draw_char == text_length[page] && page == page_number - 1) {
                // Execute selected option
                create_textbox(option_link_id[option_pos]);
                instance_destroy();
            }
            // Add AI chat activation
            else {
                // No options, just close textbox
                instance_destroy();
            }
        }
    }
...
if (is_ai_chat && ai_input_key && !waiting_for_ai && draw_char == text_length[page]) {
    input_active = true;
    keyboard_string = "";
}
...
//options (only show if not AI chat)
if (draw_char == text_length[page] && page == page_number - 1 && option_number > 0) {
    ...
    for (var op = 0; op < option_number; op++) {
        ...
        //arrow
        if option_pos == op {
            draw_text(txtbx_x, txtbx_y - op_space*option_number + op_space*op + 2, "> ");
        }
        //the option text
